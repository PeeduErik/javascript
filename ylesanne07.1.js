...
// Funktsioon keskmise temperatuuri arvutamiseks
function keskmineTemperatuur(temperatuurid) {
    let summa = 0;
    for (let i = 0; i < temperatuurid.length; i++) {
        summa += temperatuurid[i];
    }
    return summa / temperatuurid.length;
}

// Jagame kuude nimekirja kuudeks
const kuud = months.split(", ");

// Leidke iga kuu keskmine temperatuur
for (let i = 0; i < temperatures.length; i++) {
    const keskmine = keskmineTemperatuur(temperatures[i]);
    console.log(kuud[i] + ": " + keskmine + " kraadi");
}
console.log("");